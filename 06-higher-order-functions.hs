...
higherOrderSum intApplication a b = foldl (\acc n -> acc + intApplication n) 0 [a..b]
...
hoSumInts = (\x y -> higherOrderSum id x y)
...
higherOrderSequenceApplication op f z a b = foldl (\acc n -> op acc (f n)) z [a..b]
